...
class CRM_Sumfields_Form_SumFields extends CRM_Core_Form
{
    public function buildQuickForm()
    {
        ...
        if (empty($custom['fields'])) {
            ...
            CRM_Core_Session::setStatus(
                E::ts("Summary Fields is not particularly useful if CiviContribute and CiviEvent are both disabled. Try enabling at least one.")
            );
        ...
        foreach ($custom['fields'] as $k => $v) {
            $optgroup = $v['optgroup'];
            $fieldsets[$custom['optgroups'][$optgroup]['fieldset']]["active_{$optgroup}_fields"] = CRM_Utils_Array::value(
                'description',
                $custom['optgroups'][$optgroup]
            );
        ...
        foreach ($field_options as $optgroup => $options) {
            $this->addCheckBox(
                "active_{$optgroup}_fields",
                $custom['optgroups'][$optgroup]['title'],
                array_flip($options),
                null,
                null,
                null,
        ...
        if (sumfields_component_enabled('CiviContribute')) {
            ...
            $this->add(
                ...
                'financial_type_ids',
                $label,
                sumfields_get_all_financial_types(),
                true,
                ['multiple' => true, 'class' => 'crm-select2 huge']
            );
            $fieldsets[$custom['optgroups']['fundraising']['fieldset']]['financial_type_ids'] = E::ts(
                "Financial types to include when calculating contribution related summary fields."
            );
        }
        ...
        if (sumfields_component_enabled('CiviMember')) {
            ...
            $fieldsets[$custom['optgroups']['membership']['fieldset']]['membership_financial_type_ids'] = E::ts(
                "Financial types to include when calculating membership related summary fields."
            );
        }
        ...
        if (sumfields_component_enabled('CiviEvent')) {
            ...
            $this->add(
                ...
                $label,
                sumfields_get_all_participant_status_types(),
                true,
                ['multiple' => true, 'class' => 'crm-select2 huge']
            );
    ...
    public function postProcess()
    {
        ...
        if ($values['when_to_apply_change'] == 'on_submit') {
            ...
            $session::setStatus(
                E::ts("Your summary fields will begin being generated on the next scheduled job. It may take up to an hour to complete."),
                E::ts('Saved'),
                'success'
            );
        }
     ...
     **/
    public function options_to_array($options)
    {
        $ret = [];
